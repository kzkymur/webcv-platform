...
### I/O パフォーマンス最適化（2026-02-28, 2026-03-02 更新）

- ストレージ構成を変更しました。BLOB は SQLite ではなく OPFS の個別ファイルに保存します（メタデータのみを SQLite に格納）。
  - 保存先: `navigator.storage.getDirectory()/galvoweb/data/<path>`（アプリ内の論理パスをそのまま階層化して格納）。
  - 互換性: レガシー（BLOB 同梱）スキーマは非対応です。レガシーの `files` テーブルが検出された場合は破棄され、v2 スキーマ（メタデータのみ）で再作成します（データ移行は行いません）。
- `listFiles()` はメタデータ（`path`/`type`/`width`/`height`/`channels`）のみを返します。
- 実データが必要な場合は `getFile(path)` を呼び、OPFS から読み込みます。
- 連続保存のオーバーヘッド削減のため、SQLite の書き出しは 250ms デバウンス（ページ遷移/非表示時には即時フラッシュ）。BLOB の OPFS 書き込みは都度実行します。
- まとめて保存は `putMany([entries...])` を使用（BLOB を先に OPFS に書き、その後メタデータを 1 トランザクションで upsert します）。
