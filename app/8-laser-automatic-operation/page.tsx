...
function parseFigurePointsFromFile(
  file: FileEntry | null | undefined
): number[] {
  if (!file?.data) return [];
  try {
    const json = JSON.parse(
      new TextDecoder().decode(new Uint8Array(file.data))
    );
...
class AutoPreviewController {
  ...
  setDotGalvo(p: { x: number; y: number } | null) {
    this.dotGalvo = p;
  }
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    const ctrl = new AutoPreviewController(
      canvasRef.current,
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!source) {
        await ctrlRef.current?.attachVideo(null);
        return;
      }
      ...
      if (v) {
        const applyIdentity = () => {
          if (!cancelled) ctrlRef.current?.setIdentityByVideo();
        };
      ...
      const vids = devices;
      const label =
        vids.find((d) => d.deviceId === deviceId)?.label || deviceId;
      ...
      if (best) {
        ...
        if (u) ctrlRef.current?.setUndistMap(u);
      }
    })();
    return () => {
      cancelled = true;
  ...
  useEffect(() => {
    (async () => {
      ...
      setFigItems(fout);
      if (!figSel && fout[0]) setFigSel(fout[0].path);
      const seqs = files.filter((f) => f.type === "sequence");
      const sout: SeqItem[] = seqs
        .map((f) => ({
          ts: f.path.match(/(\d{4}-\d{2}-\d{2}_[0-9-]{8,})/)?.[1] || f.path,
          path: f.path,
        }))
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!path) {
        setHMat(null);
        ctrlRef.current?.setHomography(null);
        return;
      }
      ...
      try {
        ...
        if (arr && arr.length === 9) {
          ...
          setHMat(h);
          ctrlRef.current?.setHomography(h);
        } else {
          setHMat(null);
          ctrlRef.current?.setHomography(null);
        }
  ...
  async function applySequenceToSequencer(
    seqJson: SequenceV1
  ): Promise<IndependentSequencer | undefined> {
    if (!seqJson) return;
  ...
  function currentSequence(): SequenceV1 {
    ...
    try {
      const parsed = JSON.parse(jsonText) as SequenceV1;
      if (
        parsed &&
  ...
  async function saveSequence() {
    const name =
      saveName ||
      `8-laser-automatic-operation/${formatTimestamp(new Date())}.seq`;
  ...
  async function deleteSequence(path: string) {
    ...
    setSeqItems((prev) => prev.filter((x) => x.path !== path));
  }

  async function turnOffLaserAtSequenceEnd(): Promise<void> {
    const serialNow = playingSerialRef.current ?? serial;
    if (!serialNow) return;
    try {
  ...
  async function finalizePlayback(): Promise<void> {
  ...
  async function handleStart() {
    ...
    if (!parsed || !Array.isArray(parsed.fragments)) {
      alert("Invalid sequence JSON");
      return;
    }
    ...
    try {
      ...
      playingSerialRef.current = serial;
      playingSerialRef.current?.enableRealtimeGalvo();
          ...
          <div className="row" style={{ gap: 12 }}>
            ...
            >
              ...
              <input
                ...
                value={rateHz}
                onChange={(e) =>
                  setRateHz(Math.max(1, Math.min(2000, Number(e.target.value))))
                }
                style={{ width: 72 }}
              />
            </label>
            ...
            <button
              onClick={handleStart}
              disabled={!seq || isPlaying || !serialOk}
            >
              Start
            </button>
            <button onClick={handleStop} disabled={!seq || !isPlaying}>
              Stop
            </button>
          </div>
        </div>
        ...
        <div className="col" style={{ gap: 16 }}>
          ...
          <section className="col panel" style={{ gap: 8 }}>
            ...
            <div className="row" style={{ gap: 12, alignItems: "center" }}>
              <label className="row" style={{ gap: 6 }}>
                ...
                <select
                  value={figSel}
                  onChange={(e) => setFigSel(e.target.value)}
                >
              ...
              <label className="row" style={{ gap: 6 }}>
                ...
                <input
                  ...
                  step={0.1}
                  onChange={(e) =>
                    setAddDur(Math.max(0.1, Number(e.target.value)))
                  }
                  style={{ width: 110 }}
                />
              </label>
              <label className="row" style={{ gap: 6 }}>
                ...
                <input
                  ...
                  step={0.01}
                  onChange={(e) =>
                    setAddCycleSec(Math.max(0.01, Number(e.target.value)))
                  }
                  style={{ width: 100 }}
                />
              </label>
              <label className="row" style={{ gap: 6 }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const s =
                      ScanStrategies.find((x) => x.key === e.target.value) ||
                      OutlineStrategy;
                    setStrategy(s);
                  }}
                >
          ...
          <section className="col panel" style={{ gap: 8 }}>
            ...
            <canvas
              ref={tlCanvasRef}
              style={{
                width: TIMELINE_CSS_WIDTH,
                height: TIMELINE_CSS_HEIGHT,
                maxWidth: "100%",
              }}
            />
          </section>
          ...
          <section className="col panel" style={{ gap: 8 }}>
            ...
            <div className="row" style={{ gap: 8, alignItems: "center" }}>
              <label className="row" style={{ gap: 6 }}>
                ...
                <input
                  type="text"
                  placeholder="8-laser-automatic-operation/....seq"
                  value={saveName}
              ...
              >
                {seqItems.map((s) => (
                  <div key={s.path} className="file" title={s.path}>
                    ...
                    <button onClick={() => deleteSequence(s.path)}>
                      Delete
                    </button>
                  </div>
                ))}
                {seqItems.length === 0 && (
                  <div style={{ opacity: 0.7 }}>No sequences saved yet.</div>
                )}
              </div>
            </div>
          </section>
