...
async function loadRemapXY(path: string): Promise<{ xy: Float32Array; width: number; height: number } | null> {
...
async function findUndistMapXY(runTs: string, camName: string): Promise<string | null> {
  // Exact paths used by pipeline.ts
  const base = `2-calibrate-scenes/${runTs}_cam-${sanitize(camName)}`;
  const p = `${base}_remapXY.xy`;
  const f = await getFile(p);
  return f ? p : null;
}

export default function RemapPreview({ sel }: { sel: PreviewSelection | null }) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (now - lastT >= 500) { // update fps every 0.5s
  ...
  const deviceLabel = useMemo(() => {
    ...
    return d?.label || deviceId || "(unselected)";
  }, [devices, deviceId]);
  ...
  async function onSaveFrame() {
    ...
    const path = sel.kind === "inter"
      ? `3-remap-realtime/${ts}_cam-${sanitize(sel.camB)}_to_cam-${sanitize(sel.camA)}_preview.rgb`
      : `3-remap-realtime/${ts}_cam-${sanitize(sel.cam)}_undistorted_preview.rgb`;
  ...
  return (
    <div className="col" style={{ gap: 8 }}>
      <div className="row" style={{ gap: 12, alignItems: "center" }}>
        ...
        <span style={{ opacity: 0.75 }}>Expecting: <code>{sel ? (sel.kind === "inter" ? sel.camB : sel.cam) : "-"}</code></span>
        <button onClick={onSaveFrame} disabled={!sel}>Save Frame</button>
        <span style={{ opacity: 0.75 }}>FPS: {fps.toFixed(1)}</span>
      </div>
      <div className="canvasWrap">
        <canvas ref={canvasRef} style={{ width: 640, height: "auto", display: "block" }} />
      </div>
      <video ref={videoRef} style={{ display: "none" }} />
      <div style={{ fontSize: 12, opacity: 0.75 }}>
        {sel?.kind === "inter"
          ? (
            <>Output grid: <b>{sel.camA}</b> (dest). Sampling from <b>{sel.camB}</b> (src, undistorted).</>
          )
          : (
